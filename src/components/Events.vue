    ...
    >
      ...
      <v-flex xs12 sm8 md9 lg9 class="pa-2 py-4 px-3">
        ...
        <p
          ...
          style="font-size:115%;color:#757575"
        >{{eventDetails.EventDescription}}</p>

        <v-btn
          color="#1a73e8"
          v-if="eventDetails.RegistrationLink.length>0"
...
export default {
  ...
  methods: {
    getImgUrl(pic) {
      if (pic.length > 0) {
        return require("@/assets/Images/featureEvent/" + pic);
      } else {
        return require("@/assets/Images/featureEvent/imagenotfound.png");
      }
    }
  },
  filters: {
